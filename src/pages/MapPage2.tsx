...
const calculateRouteBearing = (
  ...
  const y =
    Math.sin(differenceLng) * Math.cos(endLat);
...
const isInsidePark = (lat: number, lng: number) =>
  L.latLng(PARK_CENTER[0], PARK_CENTER[1]).distanceTo(
    L.latLng(lat, lng)
  ) <= PARK_RADIUS;
...
export default function MapPage() {
  ...
  const [databaseAttractions, setDatabaseAttractions] =
  useState<ExtendedVenue[]>([]);
  const [vipItineraryTitles, setVipItineraryTitles] =
  useState<string[]>([]);
  ...
  const [realtimeBusMinutes, setRealtimeBusMinutes] =
  ...
  const arrowCount = Math.max(
    ...
    Math.min(
      8,
      Math.floor(totalDistance / footprintSpacing)
    )
  );
  ...
  for (let arrowIndex = 1; arrowIndex <= arrowCount; arrowIndex += 1) {
    const targetDistance =
      (totalDistance * arrowIndex) / (arrowCount + 1);

    const segment = segments.find(
      (item) =>
        targetDistance >= item.accumulatedStart &&
        targetDistance <=
          item.accumulatedStart + item.distance
    );

    if (!segment || segment.distance === 0) continue;

    const progress =
      (targetDistance - segment.accumulatedStart) /
      segment.distance;

    const lat =
      segment.start[0] +
      (segment.end[0] - segment.start[0]) * progress;

    const lng =
      segment.start[1] +
      (segment.end[1] - segment.start[1]) * progress;

    arrows.push({
      position: [lat, lng],
      bearing: calculateRouteBearing(
        segment.start,
        segment.end
      )
    });
  ...
  return venues.filter(
    (venue) => venue.type === currentFilter
  );
}, [currentFilter, venues, vipItineraryTitles]);

useEffect(() => {
    try {
      const storedTitles = localStorage.getItem(
  ...
  const loadAttractions = async () => {
    try {
      ...
      const convertedAttractions: ExtendedVenue[] =
  data
    .filter((item) => {
    ...
    try {
      ...
      if (realtimeBus) {
        setRealtimeBusMinutes(
          Math.max(
            1,
            Math.ceil(realtimeBus.EstimateTime / 60)
          )
        );
      } else {
        setRealtimeBusMinutes(null);
  ...
  const busDisplayMinutes =
  realtimeBusMinutes ?? nextBus?.minutesLeft ?? null;
            ...
            {selected.isBus ? (
              <div>
                ...
                >
                  ...
                  <div>
                    {busArrivalText(busDisplayMinutes)}
                  </div>
                ) : (
                  <div>{noBusText}</div>
                )}
